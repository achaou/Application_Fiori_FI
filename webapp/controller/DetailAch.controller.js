sap.ui.define([
    ...
    return BaseController.extend("finalproject.controller.DetailAch", {
        formatter: formatter,
        ...
        onInit : function(){
            ...
            this.getRouter().getRoute("objectDetailAch").attachPatternMatched(this._onObjectMatched, this);
            
            this.setModel(oViewModel , "DetailAchView")
        ...
        _onObjectMatched : function (oEvent) {
            ...
            var path= "/postedemandeSet(DemandeAchat='"+id+"',PosteDemande='"+sPostId+"')"
        ...
        _bindView : function (sObjectPath , id , sPostId) {
            var oViewModel = this.getModel("DetailAchView");
            var oModell = this.getOwnerComponent().getModel();
            var s ;
            oModell.read(sObjectPath,{success: function(odata, response){

                var s = odata.CodeArtcile
                if(s != null){
                console.log(s)
                if(s.length < 18){
                    do{
                ...
                oModell.read("/ArticleeSet('"+s+"')" ,{success: function(odata, response){
